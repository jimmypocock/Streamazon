        ...
        for account_id in account_ids:
            try:
                ...
                lambda_usage[account_id] = {
                    ...
                    'average_duration_ms': total_duration / total_invocations if total_invocations > 0 else 0,
            ...
            try:
                ...
                for metric_data in metrics:
                    if metric_data['metric'] == 'CPUUtilization':
                        ...
                        datapoints = metric_data.get('datapoints', [])
                        if datapoints:
                            free_storage = min(dp['Average'] for dp in datapoints) / (1024**3)  # Convert to GB
        ...
        try:
            ...
            for metric_data in metrics:
                ...
                if metric_data['metric'] == 'CPUUtilization':
                    ...
                    result['total_network_in_gb'] = sum(
                        dp['Sum'] for dp in datapoints
                    ) / (1024**3)
                elif metric_data['metric'] == 'NetworkOut':
                    result['total_network_out_gb'] = sum(
                        dp['Sum'] for dp in datapoints
                    ) / (1024**3)
