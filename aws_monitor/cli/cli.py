...
@click.pass_context
def cli(ctx, profile, output_json):
    """AWS Cost & Usage Monitor CLI - Quick access to AWS cost and usage data."""
...
@click.pass_context
def costs(ctx, hours, by_account, by_service, top):
    ...
    try:
        ...
        print_colored(f"Time Period: {breakdown['start_date']} to {breakdown['end_date']}")
        print_colored(f"Total Cost: {format_currency(breakdown['total_cost'])}", Colors.GREEN, bold=True)
        ...
        if breakdown['breakdown']:
            print_colored(f"\n=== Top {min(top, len(breakdown['breakdown']))} Cost Drivers ===", Colors.BOLD, bold=True)
...
@click.pass_context
def inventory(ctx, service, account):
...
def usage(ctx, service, hours):
    ...
    try:
        ...
        if service == 'EC2':
            # EC2 metrics
            total_instances = sum(acc.get('total_instances', 0) for acc in service_data.values() if isinstance(acc, dict))
            running_instances = sum(acc.get('running_instances', 0) for acc in service_data.values() if isinstance(acc, dict))

            print_colored(f"Total Instances: {total_instances}")
            print_colored(f"Running Instances: {running_instances}", Colors.GREEN)
            print_colored(f"Stopped Instances: {total_instances - running_instances}", Colors.YELLOW)
            ...
            if instance_types:
                ...
                for itype, count in sorted(instance_types.items(), key=lambda x: x[1], reverse=True)[:10]:
                    print_colored(f"  {itype}: {count}")

        elif service == 'Lambda':
            # Lambda metrics
            total_functions = sum(acc.get('function_count', 0) for acc in service_data.values() if isinstance(acc, dict))
            total_invocations = sum(acc.get('invocations_24h', 0) for acc in service_data.values() if isinstance(acc, dict))
            total_errors = sum(acc.get('errors_24h', 0) for acc in service_data.values() if isinstance(acc, dict))

            print_colored(f"Total Functions: {total_functions}")
            print_colored(f"Invocations (24h): {total_invocations:,}", Colors.GREEN)
            print_colored(f"Errors (24h): {total_errors:,}", Colors.RED if total_errors > 0 else Colors.GREEN)

            if total_invocations > 0:
                error_rate = (total_errors / total_invocations) * 100
                print_colored(f"Error Rate: {error_rate:.2f}%", Colors.RED if error_rate > 1 else Colors.GREEN)

        elif service == 'S3':
            # S3 metrics
            total_size = sum(acc.get('total_size_gb', 0) for acc in service_data.values() if isinstance(acc, dict))
            total_objects = sum(acc.get('total_objects', 0) for acc in service_data.values() if isinstance(acc, dict))
        ...
        for account_id, account_data in service_data.items():
            if isinstance(account_data, dict) and 'error' not in account_data:
                ...
                for key, value in account_data.items():
                    if key not in ['functions', 'metrics']:  # Skip detailed lists
...
@click.option('--sensitivity', type=click.Choice(['low', 'medium', 'high']), default='medium', help='Detection sensitivity')
@click.pass_context
def anomalies(ctx, days, sensitivity):
    ...
    try:
        ...
        print_colored(f"Total Anomalies: {summary.get('total_anomalies', 0)}",
        ...
        for anomaly_type, anomalies in all_anomalies.items():
            ...
            for anomaly in anomalies[:5]:  # Show top 5 of each type
                ...
                if anomaly['type'] == 'cost_spike':
                    print_colored(
                        f"⚠️  {anomaly['service']} in {anomaly.get('account_id', 'N/A')}: "
                        f"Cost increased by {format_percentage(anomaly['change_percentage'])} "
                        f"({format_currency(anomaly['baseline_cost'])} → {format_currency(anomaly['cost'])})",
                        Colors.RED
                    )
                elif anomaly['type'] == 'new_service':
                    print_colored(
                        f"🆕 New service detected: {anomaly['service']} in {anomaly.get('account_id', 'N/A')} "
                        f"(Cost: {format_currency(anomaly.get('cost_since_start', 0))})",
                        Colors.YELLOW
                    )
                elif anomaly['type'] == 'usage_anomaly':
                    print_colored(
                        f"📊 {anomaly.get('description', 'Usage anomaly detected')}",
                        Colors.YELLOW
                    )
                else:
...
def trends(ctx, days, service):
    ...
    try:
        ...
        print_colored(f"Trend: {stats['trend'].upper()}",
    ...
    try:
        ...
        for acc in accounts:
            table_data.append([
                acc['Id'],
                acc['Name'][:40],
                acc.get('Email', 'N/A')[:30],
                acc['Status']
            ])

        click.echo("\n" + tabulate(
            table_data,
            headers=['Account ID', 'Name', 'Email', 'Status'],
            tablefmt='simple'
        ))
