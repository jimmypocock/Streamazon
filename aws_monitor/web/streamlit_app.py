...
def render_cost_breakdown_chart(breakdown_data):
    ...
    if 'account_id' in df.columns and 'service' in df.columns:
        fig = px.treemap(
            ...
            values='cost',
            title=f"Cost Breakdown by Account and Service (Total: {format_currency(breakdown_data['total_cost'])})",
            color='cost',
        ...
        fig = px.bar(
            ...
            x='service',
            y='cost',
            title=f"Top 20 Services by Cost (Total: {format_currency(breakdown_data['total_cost'])})",
            color='cost',
            color_continuous_scale='RdYlBu_r'
        )
...
def render_cost_timeline(breakdown_data):
...
def render_anomaly_card(anomaly):
    ...
    if anomaly['type'] == 'cost_spike':
        ...
        message = f"Cost increased by {format_percentage(anomaly['change_percentage'])} from baseline"
    elif anomaly['type'] == 'cost_drop':
        icon = "📉"
        title = f"Cost Drop: {anomaly['service']}"
        message = f"Cost decreased by {format_percentage(anomaly['change_percentage'])} from baseline"
    elif anomaly['type'] == 'new_service':
        icon = "🆕"
        title = f"New Service: {anomaly['service']}"
        message = f"First detected {anomaly.get('first_seen', 'recently')}, costing {format_currency(anomaly.get('cost_since_start', 0))}"
...
def render_usage_metrics(usage_data):
...
def render_ec2_usage(ec2_data):
    """Render EC2 usage metrics."""
    total_instances = sum(acc.get('total_instances', 0) for acc in ec2_data.values() if isinstance(acc, dict))
    running_instances = sum(acc.get('running_instances', 0) for acc in ec2_data.values() if isinstance(acc, dict))
...
def render_lambda_usage(lambda_data):
    """Render Lambda usage metrics."""
    total_functions = sum(acc.get('function_count', 0) for acc in lambda_data.values() if isinstance(acc, dict))
    total_invocations = sum(acc.get('invocations_24h', 0) for acc in lambda_data.values() if isinstance(acc, dict))
    total_errors = sum(acc.get('errors_24h', 0) for acc in lambda_data.values() if isinstance(acc, dict))
...
def render_s3_usage(s3_data):
    """Render S3 usage metrics."""
    total_size = sum(acc.get('total_size_gb', 0) for acc in s3_data.values() if isinstance(acc, dict))
    total_objects = sum(acc.get('total_objects', 0) for acc in s3_data.values() if isinstance(acc, dict))
...
def main():
    ...
    st.markdown("Real-time monitoring of AWS costs and resource usage across your organization")
        ...
        if st.button("🔄 Initialize/Refresh", type="primary"):
            with st.spinner("Initializing AWS clients..."):
                if initialize_clients():
                    ...
                    st.session_state.data_cache = {}  # Clear cache on refresh
    ...
    with tab3:
        ...
        with col1:
            sensitivity = st.select_slider(
                ...
                value='medium',
                help="Higher sensitivity detects more anomalies but may include more false positives"
            )
        with col2:
    ...
    with tab5:
        ...
        with col2:
            forecast_days = st.number_input("Forecast days", 1, 30, 7)
